...
define([
	...
	function describe(properties, nameProperty) {
		var html = '<table class="cesium-GeoJsonDataSourceWithHolesTable">';
	...
	function processPolygon(dataSource, geoJson, geometry, crsFunction, sourceUri) {
		...
		for(var i = 1; i < geometry.coordinates.length; i++) {
			dynamicObject.holes.push(new ConstantProperty(coordinatesArrayToCartesianArray(geometry.coordinates[i], crsFunction)));
		}
	}
	...
	function processMultiPolygon(dataSource, geoJson, geometry, crsFunction, sourceUri) {
		...
		for (var i = 0; i < polygons.length; i++) {
			...
			for(var j = 1; j < polygon.length; j++) {
				dynamicObject.holes.push(new ConstantProperty(coordinatesArrayToCartesianArray(polygon[j], crsFunction)));
			}
		}
	}
	 ...
	 * @alias GeoJsonDataSourceWithHoles
	 * @constructor
	 *
	 ...
	 * var dataSource = new Cesium.GeoJsonDataSourceWithHoles();
	 ...
	 */
	var GeoJsonDataSourceWithHoles = function(name) {
		this._name = name;

		//default point
		var defaultPoint = new DynamicObject('GeoJsonDataSourceWithHoles.defaultPoint');
		...
		//default line
		var defaultLine = new DynamicObject('GeoJsonDataSourceWithHoles.defaultLine');
		...
		//default polygon
		var defaultPolygon = new DynamicObject('GeoJsonDataSourceWithHoles.defaultPolygon');
	...
	defineProperties(GeoJsonDataSourceWithHoles.prototype, {
		/**
		 * Gets a human-readable name for this instance.
		 * @memberof GeoJsonDataSourceWithHoles.prototype
		 * @type {String}
		 */
		name : {
			get : function() {
	 ...
	 */
	GeoJsonDataSourceWithHoles.prototype.loadUrl = function(url) {
	 ...
	 */
	GeoJsonDataSourceWithHoles.prototype.load = function(geoJson, sourceUri) {
		//>>includeStart('debug', pragmas.debug);
		if (!defined(geoJson)) {
		...
		if (defined(crs)) {
			...
			if (crs.type === 'name') {
				crsFunction = GeoJsonDataSourceWithHoles.crsNames[properties.name];
				if (!defined(crsFunction)) {
					throw new RuntimeError('Unknown crs name: ' + properties.name);
				}
			} else if (crs.type === 'link') {
				var handler = GeoJsonDataSourceWithHoles.crsLinkHrefs[properties.href];
				if (!defined(handler)) {
					handler = GeoJsonDataSourceWithHoles.crsLinkTypes[properties.type];
				}
				...
				crsFunction = GeoJsonDataSourceWithHoles.crsNames['EPSG:' + properties.code];
				if (!defined(crsFunction)) {
	 ...
	 * @memberof GeoJsonDataSourceWithHoles
	 * @type {Object}
	 */
	GeoJsonDataSourceWithHoles.crsNames = {
	 ...
	 * @type {Object}
	 */
	GeoJsonDataSourceWithHoles.crsLinkHrefs = {};
	 ...
	 * @type {Object}
	 */
	GeoJsonDataSourceWithHoles.crsLinkTypes = {};

	return GeoJsonDataSourceWithHoles;
});
